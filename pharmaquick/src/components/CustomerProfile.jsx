...
const CustomerProfile = () => {
  ...
  console.log(userName, userEmail, userLoc, userId);

  // Fetch user data from backend API
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await fetch('https://localhost:44301/api/customer/profile', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            // Add any necessary authentication headers, like a token if needed
  ...
  return (
    <div className="flex flex-col items-center p-6">
      ...
      {/* Show loader if user data is not yet available */}
      {{userId}==null ? (
        ...
        <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-md text-center">
          ...
          <p className="text-gray-600">{userLoc}</p>
